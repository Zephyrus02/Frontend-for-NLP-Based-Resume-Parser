...
const UploadBox = ({ filename, onFileChange, onDrop, isAuthenticated, isLoading }) => {
    ...
    return (
        <div
            ...
            onDrop={isAuthenticated ? onDrop : (e) => e.preventDefault()}
            ...
            <p>{isAuthenticated ? filename : "Login to upload your resume"}</p>
            {isAuthenticated && (
                <>
                    <input
                        type="file"
                        ...
                        style={{ display: "none" }}
                    />
                    {isLoading && <span className="loader"></span>} {/* Display loader */}
                </>
            )}
        </div>
    );
